...
Verticles are the building blocks of the Application. As a mental model, you can
compare them to _mini computers_ communicating with each other in a network and
reacting to each other's messages.

Verticles should perform small, modular/atomic tasks so that the combination of
...
pipeline could, then, look something like this (where processes are Verticles):
