...
To complete this tutorial, you should be familiar with **TODO: REPLACE**.

:::

## What you'll build

A description of what you'll build in this tutorial.
...
To do so, create a file within your `src/main/java` folder and call it
`Car.java`.
...
Within your class declaration, add a new field called `engine`, which has to
...
Adding this field lets you define an object that implements this interface and
then access it within a car object.

## Step 4: Add a `maxSpeed` field to the `Car` class

Adding the field for the car's max speed (which is an integer) works the same
way:
...
In the following tutorials, you'll learn how to create an object of your `Car`
class.
