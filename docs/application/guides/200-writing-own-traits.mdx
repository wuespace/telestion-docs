---
title: Writing own traits
# e.g., "Working with the Event Bus" or "Managing dependencies with npm"

description:
  Traits are a nice way to remove duplicate code and clean up your codebase.
  Learn how to write your own traits.
---
...
This guide teaches you on how write our own traits in Java while using the Telestion APIs.

<!-- Describe the intended target audience: -->

:::tip Is this the right guide for me?

We primarily wrote this guide for Backend Developers and everyone
that wants to write they own traits.
...
Before you implement your trait, first sit back and think about
the following questions:

### What specific problem wants your trait to solve?

First, write down the scope of your problem and try to slice it up
into specialised feature sets.
One big feature of Java interfaces is that you can import multiple of them at once.
This keeps the traits source file small, readable and maintainable.

It's not a good idea to push every feature into one and big trait.
This bloats the implementing class and pollutes the classes namespace
which then in turn can create name collisions.

### Is there already existing interfaces or traits, that I can use to support my trait?

Most of the time you want to write a trait is, when you implemented the same feature
over and over again in different parts of your Application.
The maintenance of your code shrinks and you don't have the ultimate source of thruth
because two or more implementations diverged at some point.

Try to find these multiple implementations and look on what features you currently use.
Is there an existing interface or trait that you can extend?
If that's the case, that's great, because you can keep your source code footprint
small and maintainable.

### Are there other users that want to use your trait in a different context?

It's fine to write a trait for your specialised context.
But what if you or others want to use your trait because of its neat functionality
but cannot do so because of the restrictive behaviour of your trait?

Try to think about a greater context of your trait and where you can use it.
Another neat feature of Java is method overloading.
You can define multiple methods with the same name but different argument sets.
Then, the compiler automatically selects the most suitable method for your problem
you want to solve with your imported trait.

Then you can extend your specialised method with adding more methods
with a different and wider set of arguments.

The user of your trait thanks you later. 😉

### Decision

If you don't understand all things right away, that's okay.
Take a look at the example below for better understanding.

## Example implementation of a trait

### The problem

Let's say, we have multiple verticles that use the `vertx.eventBus()` context.
They also use `JsonMessage` based messages to communicate with each other.

So they need to convert the received messages to the wanted `"JsonMessage"` type everytime.
...
You can do that better. Let's introduce a trait that allows verticles
to get better and simpler access to the event bus.
...
Hmm, how do we get access to the event bus instance? Traits should not force
...
Let's add it to your existing implementation:
...
Many exposed types on the Vert.x framework are actually Java interfaces.
The implementation side of Vert.x often import these.
This is your intercept point, if you want to write traits not only
for Verticles but also other types.
...
You start with the publishing part and add a method to publish messages
on a specific event bus channel.

That's an improvement because the verticle doesn't need to extract the event bus instance anyone.
But what's with the `JsonMessage` based messages. They must be converted to `JsonObject`
before transmitting them over the event bus.

Let's overload the `publish` method to handle `JsonMessage` messages in a special way:
...
The overloaded method only accepts `JsonMessage` as a message type
so you are safe to call `json()` on it to convert it to a `JsonObject`.
Then you pass the arguments further to the base method.

Now the user of your trait cannot forget the conversion anyone because you do it.
That's convenient.

Now we repeat this step with the call method call but you want to provide delivery options, too:
...
Now the user of your trait can provide delivery options if they like but is not forced to do so
because you provide also a method without any options. That's convenient, too.

### The `register` method

Let's repeat this procedure for the `consumer` method from the event bus instance.
Add a `register` method to your trait:
...
In this example you not only provide convenient access to the consuming side of the event bus
but also renamed the method to better specify the behaviour the user of your trait expects.

To integrate the automatic conversion from receiving messages to `JsonMessage` based messages,
you need to write a functional interface that provides the converted type to the implementing method:
...
Now, the `register` method automatically converts the incoming to the specified type
when it is provided.

The same step repeatable so that the handler not only provides the converted message body but
...
For this example, the current functionality is feasible to reduce repetition
in the sample verticles.
...
You can write even more traits for verticle.
For example the timing functions or shared data from the `vertx` context.
