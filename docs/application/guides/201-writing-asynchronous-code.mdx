...
This guide teaches you how to write asynchronous code
with the help of several language and library features.
...
or launching a HTTP server take time and can block the event loop of a verticle.
...
Sometimes, the passed function is already defined as an instance or class method inside the class
...
The second example does roughly the same with one exception that the values aren't getting summed up,
but instead multiplied with the `multiply` method.
...
Futures wrap asynchronous operations and provide a unified event-driven API to listen for a result
...
Due to the nature of futures not completing immediately, you can add event handlers
...
and, in any event, the completion handler.
...
Here, the extending verticle methods receive promises that inform the "outer" world
...
The verticle registers a success handler on the  future returned by the `server.listen` method
...
startup errors and passes them through to the "outer" world via the `startPromise.fail` method.
...
Sometimes, the future and promise types don't match.
...
and returns another future that "replaces" the old one.
...
It returns a new future which "replaces" the old one.
