...
Traits add additional functionality to your Java class
by providing default implementations you can then "import".

## Basic functionality

Traits are essentially nothing more than Java interfaces.
They provide default methods to the implementing class or interface.
You can "import" traits via the `implements` statement in your class definition.

The difference between traits and normal Java interfaces are essentially that a trait
adds functionality without forcing the implementing class
to overwrite/implement the defined methods like normal interfaces would do.

After importing a trait you have access to the provided methods from the trait
in your current scope.

Let's look on an example for a better understanding:

## A very basic logging trait

Let's say, we have a class that wants to log a message to the console:
...
And we have a trait to support the requested feature:
...
You can now implement the `WithLogging` and simply
using the default implemented method `log`:
...
The name of a trait usually contains the prefix `With`
and an action call describing what the trait provides
to the implementing class or interface. For example:
