...
In Vert.x, every verticle runs in its own thread and waits for incoming events (_event-driven_).
...
and their connected event handlers.

Most of the time this isn't a problem, especially if the event handler is compact and runs fast code.
But once the thread grabs an event handler that contains blocking or heavy-lifting code,
new events clog up the event queue, making the verticle unresponsive.

Therefore, Vert.x and Telestion use asynchronous code structures to move the blocking part away
