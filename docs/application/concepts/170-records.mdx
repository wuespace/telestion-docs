...
without having to define constructors, `toString()`, getters, etc., every single time.
...
This, automatically, also creates getters `String engine()` and `String brand()` on `Car` objects.
