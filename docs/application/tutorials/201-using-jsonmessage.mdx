...
We will extend the code from [Writing a Verticle](/application/tutorials/writing-a-verticle) with the
`JsonMessage` APIs. This stops us from having to guess about the type of message
sent and therefore poses much fewer edge cases we may need to handle.
...
<Reference to="/application/tutorials/writing-a-verticle/">Writing a Verticle (Base Code)</Reference>
