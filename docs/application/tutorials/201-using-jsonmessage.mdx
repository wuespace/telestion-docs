...
Instead of handling the `event` directly, you pass it into the `JsonMessage.on()`
...
instance of `IntegerMessage` and you can, thus, use it to further process
the data.
...
Now, all you need to do to test our code is to also publish an `IntegerMessage`
within your `main()`:
