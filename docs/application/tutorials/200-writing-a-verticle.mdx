...
In this tutorial, we write a simple [Verticle](/application/concepts/verticle)
for the Telestion ecosystem.
...
We first get an instance to the Event Bus APIs using `vertx.eventBus()`.
...
Then, we add a listener on the `"output"` address that just prints
...
Next, we deploy our `MessageTransformer` Verticle by creating an instance and hand it over to `Vertx`.
This will, in turn, call its `onStart()` method to deploy the Verticle.

The deployment process returns a future object. When it resolves,
we publish a random number (here: `3`) to the `"input"` channel.
