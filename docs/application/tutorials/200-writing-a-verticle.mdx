...
Verticles that only listen to channels without ever publishing anything
themselves can be useful, too. For example, you could write a Verticle that logs
all incoming messages to a file or saves them to the database. But in this
tutorial, you want to transform the data and "republish" it to the event bus
(this time, on a different channel called `"output"`).
