...
In this tutorial, we write a more complex and stateful dialog for a widget in
the web client.
...
In this tutorial, we'll extend the functionality from the simple dialog of the
last tutorial and add delivery options to our telecommand we want to send.
...
To be able to send delivery options with the telecommand, we need a new type
...
We also create default delivery options which we'll use later as initial value
in the React state.

Now update our `sendTC` placeholder function to accept the previously defined
delivery options:
...
To pass the user defined delivery options to our external telecommand API, we
need another React state which stores them. Then, we pass these options to the
`sendTC` function in the resolved promise:
...
Currently, the user cannot change the delivery options through the widget. To
change that, we need another action button which triggers a dialog that present
...
We wrap the existing text field in another flex container with the new action
button and align them horizontally. Give the action button a flex shrink of `0`
...
Currently, the added action button does nothing. Let's attach an event handler
that opens the dialog which presents the delivery options to the user:
...
The new dialog receives the current options as initial state and sets the
"global" options state via the setter when it succeeds.
...
we give the dialog a function that returns the content. This function receives
...
Every option gets an own check box which shows the current state of the specific
option. When the user presses on the check box, the check box emits an event
which changes only the specific option leaving the rest of the dialog state
untouched.

This comes from the nature of the `setState` function. It accepts a new partial
dialog state (e.g. one specific boolean value like in this tutorial) and shallow
merges it with current state. In the end, the changes "win" but the other values
remain intact.
...
3. the dialog content receives the dialog state (which is currently equal to the
   "global" state) and renders
...
Not only the `content` property can receive the dialog state, but also every
other property in the dialog configuration, which defines the dialog layout.

Take a look and the API definition of the `showDialog` function for further
information.
...
parts of the web client very easily.

As previously mentioned, take a look at the API reference of the dialog
function:
