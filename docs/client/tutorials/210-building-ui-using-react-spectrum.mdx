...
In this tutorial, we'll use the React Spectrum library to build our widget's UI.
...
`@wuespace/telestion-client-common` library and therefore neatly integrates into
the other parts of the client's UI.
...
We'll build our widget's UI. It should display the connection status (Connected
/ Disconnected) of three different I/O interfaces: `SAT A`, `SAT B`, and `SAT C`
and have "Reset" buttons that could, for example, trigger a Telecommand:

<Image src="img/client/my-first-widget/04.png" center />

In this tutorial, we won't connect this to any actual data source (the event
bus), that will be a part of the next tutorial.
...
Since we have three different systems, we'll extract their connection status UI
into one reusable `<Indicator />` component and use it for our three different
systems:
...
We recommend that you get familiar with both the Spectrum Design system in
general as well as the React Spectrum implementation using Adobe's resources:
...
Of course, this widget currently doesn't reflect the actual connection status.
