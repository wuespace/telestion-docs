...
In this tutorial, we install the telstion-client-cli and generate a new
...
repo's root folder. This, then, automatically bootstraps a client project into
the repo's `client` folder.

:::

This generates a folder with the chosen name that contains a ready-to-use client
project and installs all necessary dependencies.

:::info Windows execution policies

On Windows machines, you might get an error message that is about execution
policies. In this case, run the power shell as administrator and use this
command:
...
Now that we've generated our client project, we'll configure and extend our
