...
description:
  Dialogs can do much more than just showing static content. Learn how you
  can create a stateful and complex dialog with the Telestion Client Common package.
---
...
In this tutorial, we write a more complex and stateful dialog for a widget in the web client.

:::info Prerequisites

To complete this tutorial, you should be familiar with TypeScript language
and the React framework. This tutorials builds upon the code from the simple dialog tutorial.

:::

## What we'll build

In this tutorial, we'll extend the functionality from the simple dialog of the last tutorial
and add delivery options to our telecommand we want to send.
...
To be able to send delivery options with the telecommand,
we need a new type that defines the delivery options first.

Let's add an interface to our widget describing the new delivery options:

```tsx title='widget.tsx' {5-18,20-24}
import { useState } from 'react';
...
We also create default delivery options
which we'll use later as initial value in the React state.

Now update our `sendTC` placeholder function
to accept the previously defined delivery options:

```tsx title='widget.tsx' {26-28}
import { useState } from 'react';
import { ActionButton, Flex, TextField, View } from '@adobe/react-spectrum';
import { showDialog } from '@wuespace/telestion-client-common';
...
To pass the user defined delivery options to our external telecommand API,
we need another React state which stores them.
Then, we pass these options to the `sendTC` function in the resolved promise:

```tsx title='widget.tsx' {31,39}
import { useState } from 'react';
import { ActionButton, Flex, TextField, View } from '@adobe/react-spectrum';
import { showDialog } from '@wuespace/telestion-client-common';
...
Currently, the user cannot change the delivery options through the widget.
To change that, we need another action button which triggers a dialog
that present the current selection to the user.

Lets place it beside the telecommand text field:

```tsx title='widget.tsx' {45,52-55}
import { useState } from 'react';
import { ActionButton, Flex, TextField, View } from '@adobe/react-spectrum';
import { showDialog } from '@wuespace/telestion-client-common';
...
We wrap the existing text field in another flex container with the new action button
and align them horizontally.
Give the action button a flex shrink of `0` so it doesn't
collapse in the flex container.
Add some spacing between both items with `gap="size-100"`
and align them with `alignItems="end"` in the flex container.
This puts the action button on the same height as the text field input.

Now, your widget should look like:
...
Currently, the added action button does nothing.
Lets attach an event handler that opens the dialog
which presents the delivery options to the user:

```tsx title='widget.tsx' {34-40,42,60,64}
import { useState } from 'react';
import { ActionButton, Flex, TextField, View } from '@adobe/react-spectrum';
import { showDialog } from '@wuespace/telestion-client-common';
...
We create a new event handler called `onOptions` which opens the options dialog.
The new dialog receives the current options as initial state
and sets the "global" options state via the setter when it succeeds.

## Step 5: Make the options dialog interactive

Currently the dialog has no content which can interact with the dialog state.
Lets change that:

```tsx title='widget.tsx' {37-39}
import { useState } from 'react';
import { ActionButton, Flex, TextField, View } from '@adobe/react-spectrum';
import { showDialog } from '@wuespace/telestion-client-common';
...
we give the dialog a function that returns the content.
This function receives from the dialog the current dialog state (here called `state`)
and as second argument a function to change it (here called `setState`).

Now, we define the layout in the dialogs center part:

```tsx title='widget.tsx' {38-57}
import { useState } from 'react';
import { ActionButton, Flex, TextField, View } from '@adobe/react-spectrum';
import { showDialog } from '@wuespace/telestion-client-common';
...
Every option gets an own check box which shows the current state of the specific option.
When the user presses on the check box, the check box emits an event
which changes only the specific option leaving the rest of the dialog state untouched.

This comes from the nature of the `setState` function. It accepts a new partial dialog state
(e.g. one specific boolean value like in this tutorial) and shallow merges it with current state.
In the end, the changes "win" but the other values remain intact.

To wrap it up:

1. the dialog receives the "global" options state
2. the dialog it opens
3. the dialog content receives the dialog state (which is currently equal to the "global" state)
   and renders
...
Not only the `content` property can receive the dialog state,
but also every other property in the dialog configuration,
which defines the dialog layout.
...
That's it! You've successfully written your first stateful dialog!

Due to the usage of the `showDialog` function, you also can add modals in other parts
of the web client very easily.

As previously mentioned, take a look at the API reference of the dialog function:
