...
In this tutorial, you'll add a `readonly` configuration option to your widget.
...
````ts title='src/widgets/my-first-widget/model.ts'
...
````ts title='src/widgets/my-first-widget/config.tsx'
...
export function ConfigControls(props: BaseConfigControlsProps<WidgetProps>) {
	return (
		<View padding="size-200">
			<Checkbox
				isSelected={props.currentProps.readonly}
				onChange={readonly => props.onUpdate({ readonly })}
			>
...
````ts title='src/widgets/my-first-widget/index.ts'
...
Use the props' `readonly` property to deactivate the action buttons if the
property's value is true:

````ts title='src/widgets/my-first-widget/widget.tsx'
// [...]
// highlight-next-line
import { WidgetProps } from './model';
	...
	return (
		<View padding={'size-200'} height={'100%'}>
			<Flex direction="column" width="100%">
				...
				// highlight-start
				<Indicator system="SAT A" readonly={props.readonly} />
				<Indicator system="SAT B" readonly={props.readonly} />
				<Indicator system="SAT C" readonly={props.readonly} />
				// highlight-end
			</Flex>
		</View>
	);
...
// highlight-next-line
function Indicator(props: { system: string; readonly: boolean }) {
...
Add the `initialProps` field with `readonly` set to `false` to your widget's
