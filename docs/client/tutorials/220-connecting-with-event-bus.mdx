...
`useChannelLatest()` hook to display actual data. Last but not least, we'll send
an Event Bus message when an operator presses the "Reset" button.
...
2. every two seconds, send a connected status (`boolean`) to the channel address
   `system-status/[system]` for our three systems
...
When you now restart the `npm start` command (i.e., re-run
...
it (e.g., plugins), you need to restart the `tc-cli start` command (or
`npm start`) completely to use your changes.
...
Note that we default this value to `false` using `?? false`. The default value
is for when we haven't received a system status yet (which happens when
initially loading the widget).
...
Like before, we can do this directly in our `<Indicator />` component. The
...
channel. Then, we can use the Reset button's `onPress` event to call that
...
In the following tutorial, we'll take a look at adding configuration options to
your widget to allow Ground Station operators (i.e., your users) to re-configure
your widget's behavior on the fly.
