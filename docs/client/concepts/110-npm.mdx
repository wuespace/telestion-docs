...
on NodeJS) like `pip` for Python world or composer for PHP, npm is a package
...
dependencies. Dependencies themselves have a `package.json` that can also list
dependencies. Installing dependencies using the `npm install` command
...
`npm` installs the project dependencies in the `node_modules` directory. As
this, after installation, contains files that can get re-installed, you should
include the folder in your .gitignore file.
