...
Merge commits appear not to get used (to their full extent). If, for
...
For some contributions, it just isn't practical to have one merge-able branch per
...
`Closes: #issuenumber` for every closed issue into the commit's footer. Even
though fixing a bug would, initially, be a fix, the feature of a more wide-spread
refactoring (with possibly one or two breaking changes) outweighs that, and fixing a
