...
description:
  An overview of the tasks maintainers of the Telestion project modules have to
  fulfill
---
...
Telestion ecosystem have to fulfill.

<!-- Content (e.g., a table, a list, a diagram, or a definition list) -->

## CLAs

Make sure that all contributions are by contributors who have signed the CLA.
Otherwise, this project gets into a lot of legal trouble.

:::danger

If a Pull Request doesn't contain a filled checkbox or other notice about the
contributor having signed the NDA, ** don't merge it!**. If you suspect you
...
Having a conventional commit for every change enables the project's tools to
...
and merge it by rebasing it onto the `main` branch.
...
Merge commits appear not to get used (to their full extent). If, for merging a
Pull Request, you need a new conventional commit message, use Squash and Merge

:::

:::tip

For some contributions, it just isn't practical to have one merge-able branch
...
applicable conventional commit (for example, `feat: Big Refactoring`), while
putting `Closes: #issuenumber` for every closed issue into the commit's footer.
...
that, and fixing a bug is just a side-effect.
