...
Following the project's branch naming convention helps to keep things consistent
and also encourages following the conventional commit method.
...
If you don't want to read through this page, you can use the interactive "branch
name calculator" below :wink::
...
Your branch names should have the following structure:
...
It's essential to, as short as possible, describe the content of the branch (the
changes it attempts to make).

Within the Telestion Ecosystem, you should use one of two ways:
...
You can also describe the content directly in a "conventional commit like"
structure.

:::tip

Describing branch names in a way that's like conventional commits is helpful as
it already forces you to take on the "one change per branch" conventional commit
mindset.

:::

Instead of colons and scopes, use slashes (`/`). Don't care about breaking
changes within the branch name, yet.

Structure a conventional commit based branch description like this:
...
- `<description>` describes the actual change (for example, `add-send-function`)
- `<scope-name>` is a conventional commit scope present within the repository
  (cf. repository docs/guidelines for available scope names)

## Examples

Since EBNF can be a little bit abstract, here are three examples:

For the following examples, assume that your username is `@githubuser`.

:::tip Example: Fixing a long-discussed issue

You have discussed a change in issue `#83` for quite a while. You're
...
The CLI needs an extensive restructuring. This is too big to discuss in a single
issue and instead covers more than one issue.

Two or more contributors are working on this branch.
...
time than just changing the line yourself.

You omit opening an issue and instead commit the change without any issue.
