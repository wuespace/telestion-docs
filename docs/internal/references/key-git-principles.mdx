...
description:
  Critical principles for contributing to and maintaining the telestion-client
  repository
---
...
This document provides an overview of the fundamental principles you must follow
within Telestion's GitHub repositories.
...
pool of open source software. Don't use comments like _"RTFM!"_ in this
community.
...
appreciate these contributions. If the best you can do is post some discouraging
comment about why something is _"just bad"_, keep it to yourself.

The project members appreciate constructive criticism, but you should always
combine it with explicit appreciation for the time the other person has taken.

:::caution

Online comments often get misinterpreted in the worst possible way (as it's also
the safest way for the person receiving them). Misunderstandings happen far
sooner than you might think, even more so when they begin to cross cultures and
languages. Please keep that in mind when writing comments.
...
maintainers. Having one branch per change ensures the ability to merge it with a
conventional commit message, should the need arise, and guarantees correct
generation of release notes and changelog in releases.
...
required semantic kind of release at the push of a button without having to
think about it (sticking with the instantly releasable theme).

Don't worry if you might do something wrong. Even the "worst" of mistakes a
...
<Reference to="/internal/references/conventional-commit-types">
...
While, at its core, it's still "easy old JS or Java," a lot might not be obvious
from the start. Always feel free to ask about anything. If someone isn't kind in
their response, they don't belong into this project, and in 99 % of the cases,
you'll get a kind reply from someone else.
